...
class App extends Component {
  ...
  handleNext = () => {
    ...
    if(selected.index < len) {
      let prev = this.props.images.find((image,id)=> id===selected.index+1);
      ...
      <div className="gallary">
       ...
       <div className="gallary-row">
         {images.map((image, index) => (
           <div key={index}
            className={grid ? "gallary-card" : "gallary-list"}> 
            <LazyLoadImage
             alt={image}
             effect="blur"
          ...
          <Modal open={open} setState={this.handleClose} 
           images={images} selected={selected} openImage={this.openImage} 
